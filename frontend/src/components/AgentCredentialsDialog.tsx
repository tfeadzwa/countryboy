...
interface AgentCredentialsDialogProps {
  ...
  isNewAgent?: boolean;
  /** Shown in the empty PIN slot when credentials are viewed without a recoverable PIN. */
  onResetPin?: () => void;
}
...
const AgentCredentialsDialog = ({
  ...
  const showPinValue = Boolean(isNewAgent && credentials.pin);
  const items = [
    { label: "Merchant Code", value: credentials.merchant_code },
    { label: "Agent Code", value: credentials.agent_code },
    ...(showPinValue ? [{ label: "PIN", value: credentials.pin }] : []),
  ];
        ...
        <div className="px-6 pb-6 pt-4">
          <div className="grid sm:grid-cols-5 gap-4">
            <div className="sm:col-span-3 space-y-3">
              ...
              <div className={`grid gap-2 ${showPinValue ? "grid-cols-3" : "grid-cols-2"}`}>
                {items.map((item, i) => (
                  <motion.div
                    key={item.label}
              ...
              {!showPinValue && (
                <div className="rounded-xl border border-warning/30 bg-warning/10 px-3.5 py-3 space-y-2.5">
                  <div className="flex items-start gap-2.5">
                    <Lock className="h-4 w-4 text-warning shrink-0 mt-0.5" />
                    <div className="min-w-0">
                      <p className="text-sm font-semibold text-warning">PIN Hidden for Security</p>
                      <p className="text-xs text-muted-foreground mt-1 leading-relaxed">
                        PINs are encrypted and cannot be retrieved. Agent must remember their PIN or
                        request a reset.
                      </p>
                    </div>
                  </div>
                  {onResetPin && (
                    <Button
                      type="button"
                      size="sm"
                      className="h-8 w-full gap-1.5 rounded-md bg-orange-600 text-xs hover:bg-orange-700"
                      onClick={onResetPin}
                    >
                ...
                <ol className="text-[11px] text-muted-foreground space-y-2.5">
                  {(isNewAgent
                    ? [
                        ...
                        <>
                          Send via a <strong className="text-foreground/80">secure channel</strong> (in-person or
                          encrypted).
                        </>,
                        <>
